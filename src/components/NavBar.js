...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 text-white px-4 bg-black fixed">
      <div>
        <h1 className='text-5xl font-signature ml-2'>Abhiram</h1>
      </div>
      <ul className='hidden md:flex'>
        {links.map(({id, link})=> <li className='px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200' key={id}>
          <Link to={link} smooth offset={-80} duration={1000}>{link}</Link>
          </li>)}
      </ul>
